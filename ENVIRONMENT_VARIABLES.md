...
| `QUOTE_NOTIFY_EMAIL` | ✅ **(prod)** | Internal inbox(es) for order/quote/support notifications; several addresses may be comma-separated (`energy@metnmat.com,mk@metnmat.com`). `BLOG_NOTIFY_EMAIL` takes the same form for publication requests. Unset falls back to `contact@metnmat.com`, so leaving it blank quietly moves where sales leads land. |
...
| `TURNSTILE_SECRET_KEY` / `NEXT_PUBLIC_TURNSTILE_SITE_KEY` | — (recommended) | Cloudflare Turnstile on the quote / customization forms (`/api/quote`). **Set both or neither.** The site key is inlined at **build** time from the `NEXT_PUBLIC_TURNSTILE_SITE_KEY` repository *variable* in `deploy-web.yml`; the secret is runtime, in `metnmat/web/env`. With the secret set, `/api/quote` refuses submissions without a valid token — so a secret without a site key locks every visitor out, and a site key without the secret renders a widget nothing checks. Unset, the form falls back to the honeypot plus a signed timing token (`/api/quote/token`, signed with `ATTACHMENT_SIGNING_SECRET` / `INTERNAL_API_KEY`). Added 2026-09-17 after the form was used as a mail relay. |
