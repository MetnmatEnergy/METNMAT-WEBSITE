...
// Run the layout/GSAP effect before paint on the client (so cards are hidden and
// positioned before the first frame — no flash of stacked cards), but fall back
// to useEffect on the server to avoid the SSR useLayoutEffect warning.
...
export interface CardItem {
  ...
  alt?: string;
  linkUrl?: string;
  /** Optional caption rendered over the card (used for the services showcase). */
  title?: string;
  eyebrow?: string;
}
...
// Responsive card + layout sizing (the original relied on external CSS). Widths
// hold the ~400:700 portrait ratio; heights match getHeightMultiplier's breakpoints.
const FAN_CARD =
  "fan-card absolute inset-0 m-auto overflow-hidden rounded-2xl shadow-xl ring-1 ring-black/5 dark:ring-white/10 will-change-transform " +
  "h-[22rem] w-[12.5rem] min-[480px]:h-[26rem] min-[480px]:w-[15rem] min-[640px]:h-[28rem] min-[640px]:w-[16rem] " +
  "min-[768px]:h-[34rem] min-[768px]:w-[19.5rem] min-[1024px]:h-[38rem] min-[1024px]:w-[21.5rem]";
const FAN_LAYOUT_HEIGHT =
  "h-[22rem] min-[480px]:h-[26rem] min-[640px]:h-[28rem] min-[768px]:h-[34rem] min-[1024px]:h-[38rem]";

export default function CardFanCarousel({ cards }: CardFanProps) {
  ...
  useIsoLayoutEffect(() => {
    ...
    // The single "front" slot (rot 0, on top) is the only one exposed to keyboard
    // / assistive tech; the fanned & hidden duplicates behind it are decorative.
    const frontSlot = needsPagination ? HALF : totalCards >> 1;
    ...
    cardElements.forEach((card, cardIndex) => {
      ...
      // a11y: keep only the front card in the tab order + accessibility tree.
    ...
    return () => {
      ...
      if (leaveTimer) clearTimeout(leaveTimer);
      // Kill in-flight tweens so a Fast Refresh / StrictMode re-run doesn't stack them.
  ...
  return (
    <section className="relative z-20 flex w-full flex-col items-center px-4 py-4 md:px-8 lg:py-8">
      <div className="flex w-full max-w-[90rem] items-center justify-center">
        <div ref={containerRef} className={`fan-layout relative flex w-full max-w-[80rem] items-center justify-center ${FAN_LAYOUT_HEIGHT}`}>
          {cards.map((card, index) => {
                ...
                {card.title ? (
                  <>
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-4 md:p-5">
                      {card.eyebrow ? (
                        <p className="text-[11px] font-medium uppercase tracking-wide text-white/70">{card.eyebrow}</p>
                      ) : null}
                      <p className="mt-0.5 font-display text-sm font-semibold leading-tight text-white md:text-base">{card.title}</p>
                    </div>
                  </>
                ) : null}
              </div>
            ...
            return card.linkUrl ? (
              <a
                ...
                className={`${FAN_CARD} block cursor-pointer`}
              >
                {image}
              </a>
            ) : (
              <div key={index} className={FAN_CARD}>
                {image}
              </div>
            );
          })}
      ...
      {needsPagination && (
        <div className="z-30 mt-4 flex items-center justify-center gap-4 md:mt-6">
          <button className={`${ARROW_CLASSES} h-10 w-10 md:h-12 md:w-12`} onClick={() => cycle("left")} aria-label="Previous services">
            {chevron("left")}
          </button>
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`h-2 w-2 rounded-full transition-all duration-300 ${i === centerIndex ? "scale-[1.3] bg-black/70 dark:bg-white/80" : "bg-black/15 dark:bg-white/15"}`} />
            ))}
          </div>
          <button className={`${ARROW_CLASSES} h-10 w-10 md:h-12 md:w-12`} onClick={() => cycle("right")} aria-label="Next services">
            {chevron("right")}
          </button>
        </div>
      )}
