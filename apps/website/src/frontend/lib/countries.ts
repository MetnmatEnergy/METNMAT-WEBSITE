/**
 * Full ISO-3166 country list with international dialing codes, used by the
 * checkout's country dropdowns. Selecting a country auto-fills its phone code
 * (see dialFor), so the customer only types their local number.
 *
 * Sorted alphabetically by name. Dial codes are the country's primary code
 * (NANP territories carry their +1XXX area code so the prefix stays specific).
 */
