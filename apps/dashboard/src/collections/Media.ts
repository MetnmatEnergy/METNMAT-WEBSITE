 ...
 * Documents (PDF brochures/datasheets/certificates) live in the Documents
 * collection instead.
 */
export const Media: CollectionConfig = {
  ...
  access: {
    read: publicRead, // the website reads asset URLs anonymously
    create: canManageContent,
    update: canManageContent,
    delete: canManageContent,
  },
  upload: {
    // Local-disk fallback when cloud storage isn't configured (dev).
    staticDir: "media",
    mimeTypes: ["image/png", "image/jpeg", "image/webp", "image/avif", "image/svg+xml"],
    adminThumbnail: "thumbnail",
    focalPoint: true,
    // Responsive variants (requirement 14) — served via next/image with lazy loading.
    imageSizes: [
